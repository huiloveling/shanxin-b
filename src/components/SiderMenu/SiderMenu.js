...
export default class SiderMenu extends PureComponent {
  ...
  getOpenKeys =(item)=>{
      const list = this.state.openKeyList;
      list.push(item);
      this.setState({
          openKeyList:list
      })
  ...
  getSubMenuOrItem = (item) => {
    ...
    if (item.children && item.children.some(child => child.name)) {
      ...
      const openkeys = this.getOpenKeys(item.path);
      // 当无子菜单时就不展示菜单
  ...
  render() {
    ...
    const { openKeys,openKeyList } = this.state;
    ...
    // let defaultOpenKeys = this.getSelectedMenuKeys();
    return (
      <Sider
        trigger={null}
        breakpoint="lg"
