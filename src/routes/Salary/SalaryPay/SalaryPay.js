...
import { Button, Card, Col, DatePicker, Divider, Form, Row, Select, Modal } from 'antd';
...
@connect(({salaryPay, company, loading}) => ({
  salaryPay,
  company,
  loading: loading.models.salaryPay,
}))
@Form.create()
export default class SalaryPay extends PureComponent {
  state = {
    stateValue: '0',  //默认状态值
    searchParams: {
   ...
   */
  render() {
    const {salaryPay:{data}, loading}= this.props;  //正式上线使用
    ...
    const columns =[
      ...
      },
      {
        ...
        render: (text, record) => (
          <span>
            <Link to={'/salary/salaryPay/detail/'+record.id+"/"+record.company.id}>查看详情</Link> <Divider type="vertical"/>
            <a onClick={() =>{this.handlePaySalary(record)}}>确认发薪</a>
          </span>
        ),
      },];
